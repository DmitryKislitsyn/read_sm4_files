...
% modified by Dmitry Kislitsyn 10/31/2013 
%
...
%/* Our types */
object_type(19).name = '????RHK_OBJECT_FILE_HEADER';%%%%%%%-------DK
...
for j = 1:page_index_header.page_count
    ...
    for i=1:page_header(j).olc%9
        ...
        if object_list_string(j,i).objectID==10
            stringobject.offset=object_list_string(j,i).offset;%%%%%%%%% -------------fixed offset here//DK
            stringobject.size=object_list_string(j,i).size;%%%%%%%%% -------------fixed offset here//DK
        end
        if object_list_string(j,i).objectID==7
            SSpecDriftHeader.offset = object_list_string(j,i).offset;
            SSpecDriftHeader.size = object_list_string(j,i).size;
            isSpectroscopy = 1;
        end
        if object_list_string(j,i).objectID==8
            SSpecInfo.offset = object_list_string(j,i).offset;
            SSpecInfo.size = object_list_string(j,i).size;            
        end
    end
    % read string data
    %fseek(fid,(page_header(j).olc-9)*12,0);%%%%%%%%% -------------fixed offset here//DK
    fseek(fid,stringobject.offset,'bof'); 
    string_data(j) = read_string_data();
    if isSpectroscopy
        fseek(fid,SSpecDriftHeader.offset,'bof');
        spec_data(j).SSpecDriftHeader=read_SSpecDriftHeader(SSpecDriftHeader.size);
        fseek(fid,SSpecInfo.offset,'bof');
        spec_data(j).SSpecInfo=read_SSpecInfo(SSpecInfo.size);
    end
end
...
for i=1:file_header.total_page_count
    ...
    if isSpectroscopy
        metadata{i}.spec_data = spec_data(i);
    end
end
...
%%%%%%%%--------------by DK
    function out = read_SSpecDriftHeader(size)
        out.m_ftStartTime = fread(fid,1,'float32'); 
        out.m_nDriftOption = fread(fid,1,'int32'); 
        out.m_unStringCount = fread(fid,1,'uint16'); 
        count = out.m_unStringCount;
        out.m_szChannel = fread(fid,count,'uint16=>char')';
    end
    function out = read_SSpecInfo(size)
        out.m_fTime = fread(fid,1,'float32');  
        out.m_fXCoord = fread(fid,1,'float32');  
        out.m_fYCoord = fread(fid,1,'float32');  
        out.m_fdX = fread(fid,1,'float32');  
        out.m_fdY = fread(fid,1,'float32');  
        out.m_fCumulativedX = fread(fid,1,'float32');  
        out.m_fCumulativedY = fread(fid,1,'float32');  
    end
